...
%extend PIN_IK::PIN_IK {
    // Delegate URDF parsing and initialization to the standalone C++ constructor.
    PIN_IK(const std::string& base_link, const std::string& tip_link, const std::string& urdf_string,
      double timeout, double epsilon, const std::string& solve_type="Speed"){

      PIN_IK::SolveType solvetype;

      if (solve_type == "Manipulation1")
        solvetype = PIN_IK::Manip1;
      else if (solve_type == "Manipulation2")
        solvetype = PIN_IK::Manip2;
      else if (solve_type == "Manipulation3")
        solvetype = PIN_IK::Manip3;
      else if (solve_type == "Distance")
        solvetype = PIN_IK::Distance;
      else {
          if (solve_type != "Speed") {
              throw std::invalid_argument("Unknown solve_type: " + solve_type);
          }
          solvetype = PIN_IK::Speed;
      }
          PIN_IK::PIN_IK* newX = new PIN_IK::PIN_IK(base_link, tip_link, urdf_string, timeout, epsilon, solvetype);
          return newX;
    }
    ...
    // note that as a comment here https://bitbucket.org/traclabs/pin_ik/issues/18/possible-bug-with-quaternions-in-carttojnt
